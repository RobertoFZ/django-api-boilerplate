...
from django_api.api_v1.models import User, Profile
from django_api.common.pagination import PaginationHandlerMixin
from django_api.settings import URL_SERVER
from django_api.api_v1.serializers.users import UserSerializer, UserSimpleSerializer
...
class UsersView(APIView, PaginationHandlerMixin):
    ...
    def get(self, request):
        if request.user.is_anonymous or request.user.role != 'admin':
            return Response({"message": _("No tienes permisos para realizar está acción.")}, status=status.HTTP_401_UNAUTHORIZED)
        ...
        if page is not None and 'limit' in params:
            response = self.get_paginated_response(
                self.serializer_data_class(page, many=True).data)
        else:
